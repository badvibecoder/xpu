...
mkdir -p "$AI_DIR"/{ovms,openvino,pytorch,playground,cache}
mkdir -p "$SCRIPTS_DIR"/{ovms,openvino,pytorch,oneapi,playground,openclaude}
# Pre-create the cache folders so they are owned by the host user
mkdir -p "$AI_DIR/ovms/.ov_cache"
mkdir -p "$AI_DIR/cache/huggingface"
...
# ==========================================
# 3. OpenVINO Toolkit & Export Setup
# ==========================================
echo "Generating OpenVINO Toolkit environment and export scripts..."
...
echo " If successful, start OVMS with:"
echo " ./run-ovms.sh $MODEL_NAME"
echo "=========================================================="
EOF
